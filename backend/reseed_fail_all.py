...
def reseed_failed_students_full():
    with app.app_context():
        ...
        print(f"[SEED] Processing {len(target_students)} students...")
        ...
        for student in target_students:
            ...
            for row in results_data:
                ...
                # Assign 1 or 2 failed subjects in Semester 1 so it shows up immediately in the UI
                if student.register_no == "U24AN23S007": # Anjali Sharma
                    if row["code"] in ["61109", "61110"]: # Fundamentals, C
                        see_marks = 15; total = ia_marks + see_marks; grade = 'F'
                elif student.register_no == "U24AN23S010": # Pooja Menon
                    if row["code"] in ["61111", "61103"]: # Accountancy/Maths, English
                        see_marks = 16; total = ia_marks + see_marks; grade = 'F'
                elif student.register_no == "U24AN23S017": # Varun Joshi
                    if row["code"] in ["61101"]: # Kannada
                        see_marks = 12; total = ia_marks + see_marks; grade = 'F'
                elif student.register_no == "U24AN23S020": # Aishwarya Patil
                    if row["code"] in ["60124", "61109"]: # Digital Electronics, Fundamentals
                        see_marks = 14; total = ia_marks + see_marks; grade = 'F'
                else:
                    if row["code"] in ["61109"]: 
                        see_marks = 18; total = ia_marks + see_marks; grade = 'F'
            ...
            student.academic_status = "Backlog"
            if student.register_no in ["U24AN23S007", "U24AN23S010", "U24AN23S020"]:
        ...
        db.session.commit()
        print("[SEED] Successfully updated remaining 3 failed members with unique subject failures.")
