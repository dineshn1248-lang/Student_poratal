...
import { FaWhatsapp, FaSms, FaEnvelope, FaCheckCircle, FaSpinner, FaExclamationTriangle, FaUsers, FaPaperPlane, FaTimesCircle, FaBroadcastTower, FaRedoAlt, FaStethoscope, FaCode, FaAt, FaInfoCircle, FaChartPie, FaSearch } from 'react-icons/fa';
import './HOD.css';
...
export default function HODParentCommunication() {
    ...
    const checkSidStatus = async (sid) => {
        ...
        try {
            ...
            const result = await resp.json();
            if (resp.ok) {
                // Update diagnostic result seamlessly
    ...
    const successCount = logs.filter(l => l.status === 'Sent' || l.status === 'Delivered').length;
            ...
            <div style={{ background: '#1e293b', border: '1px solid #334155', borderRadius: '12px', padding: '16px 24px', marginBottom: '24px', display: 'flex', alignItems: 'center', gap: '20px', boxShadow: '0 4px 6px -1px rgba(0, 0, 0, 0.1)' }}>
                ...
                <div style={{ display: 'flex', gap: '12px' }}>
                    <div style={{ background: 'rgba(244,63,94,0.1)', border: '1px solid rgba(244,63,94,0.3)', padding: '6px 12px', borderRadius: '8px', display: 'flex', alignItems: 'center', gap: '8px' }}>
                        <FaEnvelope style={{ color: '#f43f5e' }} /><span style={{ color: '#f1f5f9', fontSize: '12px', fontWeight: '700' }}>Email: <span style={{ color: '#f43f5e' }}>Active</span></span>
                    </div>
                    <div style={{ background: 'rgba(34,197,94,0.1)', border: '1px solid rgba(34,197,94,0.3)', padding: '6px 12px', borderRadius: '8px', display: 'flex', alignItems: 'center', gap: '8px' }}>
                        <FaWhatsapp style={{ color: '#22c55e' }} /><span style={{ color: '#f1f5f9', fontSize: '12px', fontWeight: '700' }}>WhatsApp: <span style={{ color: '#fbbf24' }}>Sandbox</span></span>
                    </div>
                ...
                <div style={{ background: '#fff', padding: '30px', borderRadius: '16px', border: '1px solid #e2e8f0', display: 'flex', flexDirection: 'column' }}>
                    <h3 style={{ margin: '0 0 20px', fontSize: '18px', fontWeight: '800', color: '#0f172a', borderBottom: '1px solid #e2e8f0', paddingBottom: '12px', display: 'flex', justifyContent: 'space-between' }}>
                        ...
                        <div style={{ display: 'flex', gap: '8px' }}>
                            <button onClick={() => runDiagnostic('Email')} disabled={sending} style={{ background: '#fef3c7', color: '#92400e', border: '1px solid #fcd34d', padding: '6px 12px', borderRadius: '6px', fontSize: '12px', fontWeight: 'bold', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '6px' }}><FaAt/> Test Email</button>
                            <button onClick={() => runDiagnostic('WhatsApp')} disabled={sending} style={{ background: '#0f172a', color: 'white', border: 'none', padding: '6px 12px', borderRadius: '6px', fontSize: '12px', fontWeight: 'bold', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '6px' }}><FaStethoscope/> Test WA</button>
                        </div>
                    </h3>
                            ...
                            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: '#1e293b', padding: '16px', borderRadius: '8px' }}>
                                <div>
                                    ...
                                    <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                                        ...
                                        {diagnosticResult.result?.raw_response?.error_code && <span style={{ color: '#ef4444', fontSize: '12px', fontWeight: '700' }}>Error: {diagnosticResult.result.raw_response.error_code}</span>}
                                    </div>
                                    <div style={{ color: '#64748b', fontSize: '10px', marginTop: '6px' }}>Last Updated: {diagnosticResult.result?.raw_response?.date_updated || 'Unknown'}</div>
                                </div>
                                {diagnosticResult.result?.raw_response?.sid && (
                                    <button 
                                        onClick={() => checkSidStatus(diagnosticResult.result.raw_response.sid)}
                                        disabled={checkingSid}
                                        style={{ background: '#38bdf8', color: '#0f172a', border: 'none', padding: '8px 16px', borderRadius: '6px', fontSize: '12px', fontWeight: 'bold', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '6px' }}
                                    >
                                        <FaSearch className={checkingSid ? "fa-spin" : ""} /> {checkingSid ? "Checking..." : "Open Twilio Message Status"}
                                    </button>
                                )}
                            </div>
                        </div>
            ...
            <div style={{ background: '#fff', padding: '24px', borderRadius: '16px', border: '1px solid #e2e8f0' }}>
                ...
                <div style={{ overflowX: 'auto' }}>
                    <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
                        ...
                        <tbody>
                            {logs.length === 0 ? (
                                <tr><td colSpan="6" style={{ textAlign: 'center', padding: '40px', color: '#94a3b8', fontSize: '14px', fontWeight: '600' }}>No communications logged in database yet.</td></tr>
                            ) : (
                                logs.map(log => (
...
print("Overwrote HODParentCommunication.jsx for Twilio Sandbox Panel successfully!")
