...
import { fetchHodStudents, updateHodStudent } from '../../api/hodApi';
import './HOD.css';
...
export default function HODBacklogs() {
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const data = await fetchHodStudents();
      const bl   = data.filter(s => s.backlog_count > 0);
  ...
  return (
    <div className="hod-page-content">
                    ...
                    <td style={TD}>
                      <div style={{ display:'flex', alignItems:'center', gap:8 }}>
                        ...
                        <button onClick={() => openEditModal(student)}
                          ...
                          style={{ display:'flex', alignItems:'center', gap:6, padding:'7px 12px', background:'#f0fdf4', color:'#16a34a', border:'1px solid #bbf7d0', borderRadius:8, fontSize: 14, fontWeight:700, cursor:'pointer', transition:'all 0.2s' }}
                          onMouseEnter={e => { e.currentTarget.style.background='#16a34a'; e.currentTarget.style.color='#fff'; }}
                          onMouseLeave={e => { e.currentTarget.style.background='#f0fdf4'; e.currentTarget.style.color='#16a34a'; }}>
                          <FaEdit style={{ fontSize: 12 }} /> Edit
                        </button>
      ...
      {editModal && editStudent && (
        <div style={{
          position:'fixed', inset:0, background:'rgba(8,18,38,0.55)', zIndex:9999,
          display:'flex', alignItems:'center', justifyContent:'center', backdropFilter:'blur(4px)'
        }} onClick={e => { if (e.target === e.currentTarget) closeEditModal(); }}>
          <div style={{ background:'#fff', borderRadius:20, padding:'36px 40px', width:460, boxShadow:'0 20px 60px rgba(0,0,0,0.2)', position:'relative' }}>

            {/* Close */}
            <button onClick={closeEditModal}
              style={{ position:'absolute', top:16, right:16, background:'#f1f5f9', border:'none', borderRadius:'50%', width:32, height:32, display:'flex', alignItems:'center', justifyContent:'center', cursor:'pointer', color:'#64748b', fontSize: 14 }}>
              <FaTimes />
            </button>
            ...
            <div style={{ marginBottom:24 }}>
              ...
              <p style={{ color:'#64748b', fontSize: 14, marginTop:6 }}>
                Update parent contact details for <strong style={{ color:'#2563eb' }}>{editStudent.full_name}</strong>
                <span style={{ marginLeft:8, background:'#f1f5f9', color:'#475569', fontSize: 12, padding:'2px 8px', borderRadius:20, fontWeight:700 }}>{editStudent.register_no}</span>
              </p>
            </div>

            {/* Parent Phone */}
            <div style={{ marginBottom:20 }}>
              <label style={{ display:'block', fontSize: 14, fontWeight:800, color:'#374151', marginBottom:8, textTransform:'uppercase', letterSpacing:0.5 }}>
                <FaPhoneAlt style={{ marginRight:6, color:'#6366f1' }} />Parent Mobile Number
              </label>
              <input
                type="tel"
                value={editPhone}
                ...
                style={{ width:'100%', height:46, padding:'0 16px', border:'1.5px solid #e2e8f0', borderRadius:12, fontSize: 14, fontWeight:600, outline:'none', color:'#1e293b', background:'#f8fafc', boxSizing:'border-box', transition:'border-color 0.2s' }}
                onFocus={e => e.target.style.borderColor='#6366f1'}
                onBlur={e => e.target.style.borderColor='#e2e8f0'}
              />
            </div>
            ...
            <div style={{ marginBottom:28 }}>
              <label style={{ display:'block', fontSize: 14, fontWeight:800, color:'#374151', marginBottom:8, textTransform:'uppercase', letterSpacing:0.5 }}>
                <FaEnvelope style={{ marginRight:6, color:'#6366f1' }} />Parent Email Address
              </label>
              <input
                type="email"
                value={editEmail}
            ...
            {/* Save message */}
            {saveMsg && (
              <div style={{ marginBottom:16, padding:'10px 16px', background: saveMsg.startsWith('✅') ? '#ecfdf5' : '#fef2f2', color: saveMsg.startsWith('✅') ? '#065f46' : '#b91c1c', borderRadius:10, fontSize: 14, fontWeight:700 }}>
                {saveMsg}
              </div>
            )}
            ...
            <div style={{ display:'flex', gap:12 }}>
              <button onClick={closeEditModal}
                style={{ flex:1, height:46, background:'#f1f5f9', border:'none', borderRadius:12, fontSize: 14, fontWeight:700, color:'#64748b', cursor:'pointer' }}>
                Cancel
              </button>
              <button onClick={handleSaveContact} disabled={saving}
                style={{ flex:2, height:46, background: saving ? '#a5b4fc' : '#4f46e5', border:'none', borderRadius:12, fontSize: 14, fontWeight:800, color:'#fff', cursor: saving ? 'not-allowed' : 'pointer', display:'flex', alignItems:'center', justifyContent:'center', gap:8, transition:'background 0.2s' }}>
                {saving ? <><FaSpinner className="fa-spin" /> Saving...</> : <><FaSave /> Save Contact Details</>}
              </button>
            </div>
          </div>
        </div>
