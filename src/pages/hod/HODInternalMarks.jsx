...
export default function HODInternalMarks() {
    ...
    const loadData = async () => {
        ...
        try {
            ...
            const resp = await fetch(`${import.meta.env.PROD ? 'https://student-poratal.onrender.com/api' : 'http://localhost:5000/api'}/hod/internal-marks?semester=${selectedSem}`, {
    ...
    const openEditModal = async (subject) => {
        ...
        try {
            const token = localStorage.getItem('token');
            const resp = await fetch(`${import.meta.env.PROD ? 'https://student-poratal.onrender.com/api' : 'http://localhost:5000/api'}/hod/internal-marks/${subject.id}/students`, {
                headers: { 'Authorization': `Bearer ${token}` }
            });
            const data = await resp.json();
            if (resp.ok) {
    ...
    const saveInternalMarks = async (e) => {
        ...
        try {
            const token = localStorage.getItem('token');
            const resp = await fetch(`${import.meta.env.PROD ? 'https://student-poratal.onrender.com/api' : 'http://localhost:5000/api'}/hod/internal-marks/update`, {
                method: 'POST',
                headers: {
