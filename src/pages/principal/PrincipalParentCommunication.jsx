...
export default function PrincipalParentCommunication() {
    ...
    const handleSend = async (method) => {
        ...
        try {
            const token = localStorage.getItem('token');
            const resp = await fetch(`${import.meta.env.PROD ? 'https://student-poratal.onrender.com/api' : 'http://127.0.0.1:5000/api'}/hod/parent-communication/send`, {
                method: 'POST',
                headers: {
    ...
    const handleRetry = async (logId) => {
        const token = localStorage.getItem('token');
        try {
            const resp = await fetch(`${import.meta.env.PROD ? 'https://student-poratal.onrender.com/api' : 'http://127.0.0.1:5000/api'}/hod/parent-communication/retry/${logId}`, {
                method: 'POST',
                headers: { 'Authorization': `Bearer ${token}` }
            });
            const result = await resp.json();
